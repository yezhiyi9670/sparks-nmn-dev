...
export default {
	...
	'editor': flattenI18nData({
		...
		"inspector": {
			...
			"determine_props": {
				"title": "基调与拍速测定",
				"tips": "这是一个向导式的工具，用来帮助在听写旋律时确定准确的基调和拍速。暂不支持微分音。",
				"base": {
					"title": "基调测定",
					"tips": {
						"1": "在播放待听写旋律的同时按下面的音符，找出一个你能在旋律中确定相对音高的音符并选中。",
						"2": "这个音符在旋律中听起来是",
						"3": "因此，这部分旋律应当使用的基调是"
					},
					"include_halfkey": "包含 24 平均律微分音"
				},
				"speed": {
					"title": "拍速测定",
					"tips": {
					...
					"instruction": "现在，通过点击屏幕或按键盘上的空格、句点或 Enter 键打拍，尽可能保证打拍稳定。",
					"too_few": "目前所打的拍数太少，很可能不足以确定准确拍速。",
					"reset": "重置"
				}
			},
			"instrument_test": {
